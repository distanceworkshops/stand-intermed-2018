...
`RStudio` is a graphical user interface (GUI) to `R`; a major advantage of using `RStudio` is that it makes it easy to type `R` code into a script window which can be saved and also sent to the `R` command line to be run. This section describes how to open and use the user-interface `RStudio`.

Check that `RStudio` and `R` have been installed on your computer; for this course we are using versions INCLUDE VERSIONS. When you open an `RStudio` session, the window will likely be similar to that shown in Figure 1. 
...
+ generated 50 random values from a Normal distribution (using the `rnorm` function) that has a mean of 0 and standard deviation of 3 (i.e. $N(\mu=0,  \sigma^2=3.5^2)$) 
...
Listing all the data is not very helpful, especially if there is a lot of it, but there are ways to display a subset of it. 
...
if (answer) {
  cat('The minimum value is ',format(min(dists)),'.','\n')
  cat('Remember negative values are on the left of the trackline.')
}
```
\color{black}
...
```{r, echo=F}
# Maximum value 
if (answer) cat('The maximum value is ',format(max(dists)),'.','\n')
...
# Plot a histogram
if (answer) hist(dists)
```

Functions often have arguments which are specified by default and hence do not need to be specified by the user, unless the user wishes to change the default values. For example, the number of intervals in a histogram can be changed using the `breaks` argument. 
...
```{r, echo=T, fig.height=3.5, fig.width=3.5}
posdists <- abs(dists)
```
...
if (answer) hist(posdists, xlab="Distance")
```
...
\color{blue}
```{r, echo=F}
if (answer) cat('Estimated probability of detection is ',format(phat),'.')
```
\color{black}
...
Selecting a subset of data is a frequent data manipulation that is required and as shown above, square brackets (i.e. `[]`) are frequently used. In a distance sampling context, we may want to exclude distances in the righthand tail of the distribution.
...
How does this affect the probability of detection? 

\color{blue}
```{r, echo=F, fig.width=3.5, fig.width=3.5}
if (answer) {
...
There are hundreds of R packages and so for efficiency the more specialised packages are not installed as standard when R is downloaded onto your computer. To check whether the  `Distance` package (Miller 2017) is installed on your laptop, click on the 'Packages' tab (lower right pane) and see if it is listed. If so, then either click in the tick box or load it using the command
...
If we want to save the output so that we can interrogate it later, then we can assign the output to an object (`dsmodel`) as follows: 

`dsmodel <- ds(posdists, key="hn", adjustment=NULL)`

```{r, echo=F, warning=F, message=F}
# Fit a half-normal detection function
if (answer) dsmodel <- ds(posdists, key="hn", adjustment=NULL, quiet=T)
```

To examine the detection function, use the`summary` function. The `summary` function is a generic function and selects a relevant 'summary' based on the object. 
...
if (answer) plot(dsmodel)
```

To truncate the distances, then the `truncation` argument in `ds` can be used:

`dstrunc <- ds(posdists, key="hn", truncation=6)`

`summary(dstrunc)`

\color{blue}
```{r, echo=F, message=F, fig.height=3.5, fig.width=3.5}
# Fit a half-normal detection function, right truncation distances at 6
if (answer) {
  dstrunc <- ds(posdists, key="hn", truncation=6, quiet=T)
  print(summary(dstrunc))
  plot(dstrunc)
}
```
\color{black}
...
In the next practical, we will use real data. 
